...
function Test-TimeScapeDeviceCoverage(
    ...
    $device = $Run.device
    if ($device.formFactor -notin $FormFactors -or $device.windowMode -notin $WindowModes) {
        return $false
    }
...
function Require-TimeScapeDeviceCoverage(
    ...
    if (@($Runs | Where-Object {
        $_.result -eq "pass" -and
            (Test-TimeScapeDeviceCoverage $_ $FormFactors $WindowModes $Orientations $Postures)
    }).Count -eq 0) {
...
foreach ($run in $runs) {
    ...
    Assert-OnlyProperties $device @("manufacturer", "model", "androidApi", "build", "formFactor", "windowMode", "orientation", "posture", "installType") "run.device"
...
if ($RequireTimeScapeMvp) {
    ...
    Require-TimeScapeDeviceCoverage $timeScapeRunsById["feature-timescape-mvp-compact-portrait"] "feature-timescape-mvp-compact-portrait" "phone/compact/portrait" @("phone") @("compact") @("portrait")
    Require-TimeScapeDeviceCoverage $timeScapeRunsById["feature-timescape-mvp-compact-landscape"] "feature-timescape-mvp-compact-landscape" "phone/compact/landscape" @("phone") @("compact") @("landscape")
    Require-TimeScapeDeviceCoverage $timeScapeRunsById["feature-timescape-mvp-folded-cover"] "feature-timescape-mvp-folded-cover" "foldable/compact/cover" @("foldable") @("compact") @() @("cover")
    Require-TimeScapeDeviceCoverage $timeScapeRunsById["feature-timescape-mvp-expanded-adaptive"] "feature-timescape-mvp-expanded-adaptive" "foldable expanded flat, book, or tabletop" @("foldable") @("expanded", "split-screen") @() @("flat", "book", "tabletop")
    Require-TimeScapeDeviceCoverage $timeScapeRunsById["feature-timescape-mvp-expanded-adaptive"] "feature-timescape-mvp-expanded-adaptive" "tablet or desktop expanded/resizable" @("tablet", "desktop") @("expanded", "split-screen")
    Require-TimeScapeDeviceCoverage $timeScapeRunsById["feature-timescape-mvp-performance"] "feature-timescape-mvp-performance" "compact performance" @("phone") @("compact") @("portrait", "landscape")
    Require-TimeScapeDeviceCoverage $timeScapeRunsById["feature-timescape-mvp-performance"] "feature-timescape-mvp-performance" "expanded performance" @("foldable", "tablet", "desktop") @("expanded", "split-screen")
}
